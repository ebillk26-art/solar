...
# Load fault information from JSON
def load_fault_info():
    """Load fault metadata from JSON file"""
    fault_info_path = Path('models/fault_info.json')
...
FAULT_INFO = load_fault_info()

@st.cache_resource
def load_model():
    """Load the YOLOv8 model"""
    model_path = Path('models/solar_fault_model.pt')
    ...
    if model_path.exists():
        ...
        st.info("Please ensure models/solar_fault_model.pt exists in the repository")
        return None
...
if page == "🔍 Analyze Image":
    ...
    with col2:
        ...
        if sample_dir.exists() and any(sample_dir.glob('*.jpg')):
            ...
            st.info("📁 No sample images available. Upload your own thermal image!")
